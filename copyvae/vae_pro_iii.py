...
class ActiveCopyLayer(keras.layers.Layer):
    """ compute expected active copy """

    def __init__(self, max_cp, **kwargs):
        super(ActiveCopyLayer, self).__init__(**kwargs)
        self.max_cp = max_cp
    ...
    def call(self, x):
        w = self.w
        alpha = self.act(w)
        binom = tfp.distributions.Binomial(total_count=x, probs=alpha)
        act_mat = tf.zeros_like(x)
        print("this is x")
        print(x)
        for c in range(self.max_cp +1):
            tmp_mat = binom.prob(c)
            #print(tmp_mat)
            act_mat = act_mat + tmp_mat
        #print(act_mat)
        return act_mat
...
class DecoderCategorical(keras.models.Model):
    ...
    def __init__(self, original_dim, intermediate_dim,
        ...
        self.catsampling = GumbelSoftmaxSampling()
        self.alpha_layer = ActiveCopyLayer(self.max_cp)
    ...
    def call(self, inputs):
        ...
        copy, cat_prob = self.catsampling(rho_list, bin_size=self.bin_size) # batch x gene, batch x gene x copies
        mu = self.alpha_layer(copy) 
...
with tf.device(d):
    ...
    model = CopyVAE(x_train.shape[-1], 128, 10, 25, 6)
